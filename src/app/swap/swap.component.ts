  ...
  calculateSingleSwap() {
    ...
    if (poolDetail) {
      ...
      if (this.sourceAssetUnit) {
        this.targetAssetUnit = (afterFee.amount().isLessThan(0)) ? bn(0) : afterFee.amount();
      } else {
        this.targetAssetUnit = (this.sourceAssetUnit) ? (afterFee.amount().isLessThan(0)) ? bn(0) : afterFee.amount() : null;
      }
