...
export class ThorchainPricesService {
  ...
  estimateRunePrice(pools: PoolDTO[]): number {
    ...
    const stableCoins = pools.filter( (pool) => {
      ...
      return (asset.chain === 'ETH' && asset.ticker === 'USDT')
      || (asset.chain === 'BNB' && (asset.ticker === 'USDT' || asset.ticker === 'BUSD'));
    ...
    const sortedStableCoins = stableCoins.sort( (a, b) => +b.runeDepth - +a.runeDepth );
